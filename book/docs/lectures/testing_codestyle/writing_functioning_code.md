...
Another scenario that you could imagine is if you wanted to test your code against a large number of examples (say 10 or even 100 different examples). In that case you have to copy and paste the code above a lot and this would go againt the DRY (Don't Repeat Yourselves) principle. 
...
```
In fact, this is a good opurtunity to show the unpacking operator in python. In this case we can acutuall do the following
```python
arg = (1, 2, 0)
u = Vector3D(*arg)
```
The `*` is called an unpacking operator so that `*arg` upacks the list so that `Vector3D(*arg)` is identical to `Vector3D(arg[0], arg[1], arg[2])` when `arg` is a list of three elements.
...
```
This will fix the copy-pasing issue since any new examples could now just be added to the list. However, if the first test fails it will still not run the remaining tests.
...
which will ouput the following
```
...
#### Runing only a subset of your tests with pytest
...
Sometimes you might exerience that your tests is not behaving as expected. The natural thing to do in this case is to add a few print statements to get some more information. However, it you do this and then try to run `pytest` you will not see anything being printed. This is because `pytest` turns off all printing by default. You can turn this on again if you pass in the `-s` option, e.g
```
python -m pytest -s
```
and this can of course also be combined with other options for example if we add a print statuement to the `test_square` function
```python
@pytest.mark.parametrize("x, x2", [(0, 0), (1, 1), (2, 4), (3, 9)])
